...
### 7️⃣ Online Exam Result Normalization System
**Problem Statement:**  
Normalize online exam results based on student performance and overall class statistics.
...
**Tasks:**
- Add grace marks if score lies between **33–35**
- Normalize marks if **class average < 50**
- Recalculate student ranks after score updates
- Detect abnormal score inflation after normalization

**Edge Cases:**
- Marks exceeding maximum limit
- Multiple students with same total marks
- Rank recalculation after mark updates
- Student already passed before grace marks
- Empty subject list for a student

---


### 8️⃣ Hospital Emergency Triage System
**Problem Statement:**  
Manage patient prioritization efficiently in a hospital emergency ward.
...
- Assign priority based on:
  ...
  - Patient age (senior citizens first)
  - Arrival time
- Update patient severity dynamically
- Track patient waiting time in the queue

**Edge Cases:**
- Patients with same priority values
- Priority update after enqueue operation
- Patient removed unexpectedly from the queue
- Starvation of low-priority patients
- Empty priority queue scenario

---

### 9️⃣ Employee Leave Validation System
**Problem Statement:**  
Design a leave management system to validate, track, and report employee leaves efficiently.

**Collections Used:**
- `Dictionary<int, List<Leave>>` (maps EmployeeId → List of Leaves)
- `HashSet<DateTime>` (stores holidays for validation)

**Tasks:**
- Validate leave requests based on:
  - Maximum **2 consecutive leaves**
  - Holidays should **not count** as leave
  - Maximum **12 casual leaves per year**
- Reject overlapping leave requests
- Generate leave balance report for employees

**Edge Cases:**
- Leave spanning **weekends and holidays**
- Year-end leave spanning **December → January**
...
### 10️⃣ Smart Shopping Cart System
**Problem Statement:**  
Build a shopping cart system that efficiently manages items, discounts, and checkout using collections.

**Collections Used:**
- `Dictionary<int, CartItem>` (stores ProductId → CartItem)
- `List<Offer>` (stores available discount rules)

**Tasks:**
- Add items to the cart (increase quantity if already present)
- Apply offers:
  - Buy 2 Get 1 free (same product)
  - 10% discount if total > ₹5000
  - Free delivery if item count ≥ 5
- Apply only the **best applicable offer**
- Generate final bill with detailed breakdown

**Edge Cases:**
- Cart value exactly ₹5000
- Multiple offers applicable at the same time
- Buy 2 Get 1 offer when quantity = 2
- Removing an item reduces eligibility for an offer
